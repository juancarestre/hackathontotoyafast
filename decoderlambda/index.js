...
exports.handler = async (event) => {
    ...
    if (!getParams.Key.includes('vin')) {
        ...
        return rekognition.detectText(params).promise().then(async res => {
            ...
            await sendInfoToSocket(dynamoDB, `car license plate detected: ${carExtractedData.plate}`)
            ...
            if (result.Items.length === 0) {
                ...
                let vinData = vindecoder(result.Items[0].vin).decode()
...
function vin_finder(TextDetections) {
    for (let i = 0; i < TextDetections.length; i++) {
        if (TextDetections[i].DetectedText.length == 17) {
